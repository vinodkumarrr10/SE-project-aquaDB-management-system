...
$sql = "SELECT item_id, item_model, quantity_in_stock, rent_price FROM Inventory_Items where rent_price > 0"; 
//if rent_price == 0, the item is not eligible for renting
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $items[] = array(
            ...
            'rent_stock' => $row['quantity_in_stock']
        );
    }
}
...
<body>
    <div class="container">
        ...
        <div class="data">
            <div class="top">
                ...
                <p   style="font-size: 3em;color: lightslategrey;">Rental Dashboard</p>
                <a href="inventoryhome.php"><button class="btnlogin-popup">Home</button></a>
            </div>

            <div class="popup-form" id="popup-form" >
                <div class="form-container">
                    <h2 style="text-align: center;"><b><u>Rent Information</u></b></h2>
                    <form id="rentinfo" action="#" method="post">
                        <div class="form-group">
                        <label for="rentPeriod">No of days:
                            <input type="number" id="rentPeriod" name="rentPeriod" required min="1" max="7" onchange="calculateTotal()">
                        </label>
                        <p>
                            <strong>Terms and Conditions:</strong>
                            <br>
                            1. Products can only be rented for a period of 7 days.
                            <br>
                            2. A minimum charge of 1 day is applicable for all rentals.
                            <br>
                            3. For the remaining 7 days, an additional charge of 25% of the product's price will be added for each day.
                            <br>
                            4. If the product is not returned within the rental period, a fine of Rs.1500/- will be charged.
                        </p><br>
                        <h3>Total Amount: <span id="totalAmount">0</span></h3>
                                                        
                        </div>
                        <button type="submit" onclick="reload">next</button>
            ...
            <div class="popup-form2" id="popup-form2" style="display: none;">
                <div class="form-container">
                        <h2>Enter Customer Details</h2><br>
                        <form id="customerForm" action="rentconnect.php" method="post">
                            <div class="form-group">
                        ...
                        <button id="close-customer-popup">Back</button>
    ...
    <script>
         const closePopupButton2 = document.getElementById("close-customer-popup");
        const nextButton = document.querySelector("button[type=submit]");
        const customerFormPopup = document.getElementById("popup-form2"); // Assuming you have a second popup for the customer form

        nextButton.addEventListener("click", (event) => {
            event.preventDefault(); // Prevent the default form submission

            // Show the customer form popup
            showCustomerFormPopup();
        });

        function showCustomerFormPopup() {
            customerFormPopup.style.display = "flex";
        }
                    ...
                    resultElement.innerHTML = `
                        <div class='box'>
                            ...
                            <div class='bottom'>
                                ...
                                <h2>Availability:${item.rent_stock}</h2>
                            </div> 
                            <button class="add-to-cart" data-id="${item.id}" data-model="${item.model}" data-price="${item.price}">
                               
                                <a href="booking.php?id=<?php echo($item_id) ?>">Rent</a>
                            </button>
                        </div>
                    `;
        ...
        closePopupButton2.addEventListener("click", () => {
            customerFormPopup.style.display = "none";
        });
    });
